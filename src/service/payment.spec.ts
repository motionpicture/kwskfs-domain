...
describe('payPecorino()', () => {
    ...
    it('Pecorinoサービスが正常であれば決済できるはず', async () => {
        ...
        sandbox.mock(kwskfs.pecorinoapi.service.transaction.Pay.prototype).expects('confirm').once().resolves();
        sandbox.mock(kwskfs.pecorinoapi.service.transaction.Deposit.prototype).expects('start').once().resolves(pecorinoDepositTransaction);
        sandbox.mock(kwskfs.pecorinoapi.service.transaction.Deposit.prototype).expects('confirm').once().resolves();

        const result = await kwskfs.service.payment.payPecorino(
    ...
    it('Pecorinoサービスでエラーが発生すればアクションにエラー結果が追加されるはず', async () => {
        ...
        sandbox.mock(actionRepo).expects('giveUp').once()
            .withArgs(existingTransaction.potentialActions.order.potentialActions.payPecorino.typeOf, action.id).resolves(action);
        sandbox.mock(kwskfs.pecorinoapi.service.transaction.Pay.prototype).expects('confirm').once()
            .rejects(confirmPecorinoTransactionResult);
        sandbox.mock(kwskfs.pecorinoapi.service.transaction.Deposit.prototype).expects('start').never();
    ...
    it('注文取引結果が未定義であればNotFoundエラーとなるはず', async () => {
        ...
        sandbox.mock(transactionRepo).expects('findPlaceOrderById').once().withExactArgs(transaction.id).resolves(transaction);
        sandbox.mock(actionRepo).expects('start').never();
        sandbox.mock(kwskfs.pecorinoapi.service.transaction.Pay.prototype).expects('confirm').never();
