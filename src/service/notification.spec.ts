...
describe('report2developers()', () => {
    beforeEach(() => {
        process.env.KWSKFS_DEVELOPER_LINE_NOTIFY_ACCESS_TOKEN = 'accessToken';
    });
    ...
    it('LINE Notifyのアクセストークンを環境変数に未設定であれば、エラーになるはず', async () => {
        delete process.env.KWSKFS_DEVELOPER_LINE_NOTIFY_ACCESS_TOKEN;
        ...
        const result = await kwskfs.service.notification.report2developers('', '', imageThumbnail, imageFullsize)()
            .catch((err) => err);
    ...
    it('LINE Notifyの200を返さなければ、エラーになるはず', async () => {
        ...
        const result = await kwskfs.service.notification.report2developers('', '')()
            .catch((err) => err);
    ...
    it('imageThumbnailがURLでなければ、エラーになるはず', async () => {
        ...
        const result = await kwskfs.service.notification.report2developers('', '', imageThumbnail)()
            .catch((err) => err);
...
describe('sendEmailMessage()', () => {
    ...
    it('SendGridの状態が正常であればエラーにならないはず', async () => {
        ...
        const result = await kwskfs.service.notification.sendEmailMessage(<any>sendEamilMessageActionAttributets)({ action: actionRepo });
