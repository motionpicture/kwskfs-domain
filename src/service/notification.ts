...
const debug = createDebug('kwskfs-domain:service:notification');
...
export function report2developers(subject: string, content: string, imageThumbnail?: string, imageFullsize?: string): Operation<void> {
    return async () => {
        if (process.env.KWSKFS_DEVELOPER_LINE_NOTIFY_ACCESS_TOKEN === undefined) {
...
${content}`
        ...
        return new Promise<void>((resolve, reject) => {
            request.post(
                {
                    url: LINE_NOTIFY_URL,
                    auth: { bearer: process.env.KWSKFS_DEVELOPER_LINE_NOTIFY_ACCESS_TOKEN },
                    form: formData,
                    json: true
                },
                (error, response, body) => {
